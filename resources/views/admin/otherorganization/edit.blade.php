...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow-sm mb-6 dark:bg-slate-800">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-xl font-semibold text-gray-800 dark:text-white">Edit Other Organization</h1>
        ...
        <form action="{{ route('otherorganizations.update', [$organization->slug, $otherOrganization->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $otherOrganization->name) }}"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="short_name" id="short_name"
                                value="{{ old('short_name', $otherOrganization->short_name) ? strtoupper(old('short_name', $otherOrganization->short_name)) : '' }}"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="established_date" id="established_date"
                                value="{{ old('established_date', $otherOrganization->established_date ? $otherOrganization->established_date->format('Y-m-d') : '') }}" 
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="url" name="url" id="url" value="{{ old('url', $otherOrganization->url) }}"
                ...
                <div class="mt-6">
                    ...
                    <textarea name="mission" id="mission" rows="3" placeholder="Organization mission statement..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-800 focus:border-blue-800 dark:text-black">{{ old('mission', $otherOrganization->mission) }}</textarea>
                    @error('mission')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description" id="description" rows="4" placeholder="Brief description about the organization..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-800 focus:border-blue-800 dark:text-black">{{ old('description', $otherOrganization->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 mt-6">
                    ...
                    <div
                        ...
                        <input type="hidden" name="image" id="image"
                            value="{{ old('image', $otherOrganization->image) }}">

                        <!-- Existing image preview -->
                        @if($otherOrganization->imageMedia)
                        <div id="existing-image-preview" class="mb-4 w-full">
                            <div class="relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-sm overflow-hidden w-full">
                                ...
                                <div class="flex items-center justify-center p-4 bg-gray-50 dark:bg-gray-700/50">
                                    <img src="{{ $otherOrganization->imageMedia->url }}" alt="Current Image" class="max-w-full h-48 object-contain rounded-md shadow-sm border border-gray-200 dark:border-gray-600">
                                </div>
                            </div>
                        </div>
                        @endif
                        ...
                        <button type="button" id="select-featured-image"
                            ...
                            {{ $otherOrganization->imageMedia ? 'Change Featured Image' : 'Select Featured Image' }}
                        </button>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateFeaturedImagePreview(url) {
                ...
                $('#remove-featured-image').removeClass('hidden').show();
            ...
            function clearFeaturedImage() {
                ...
                $('#image-preview').empty();
                $('#existing-image-preview').hide();
                $('#remove-featured-image').addClass('hidden').hide();
