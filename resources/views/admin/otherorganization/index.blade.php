...
@section('content')
    @include('components.breadcrumb', [
        'breadcrumbs' => [['title' => 'Other Organizations', 'url' => null]],
    ])
    ...
    <div class="premium-card bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700">
        <div
            ...
            <div class="premium-card-title flex items-center mb-4 md:mb-0">
                <div
                    ...
                    <i class="fas fa-building text-white text-md"></i>
                </div>
                <div class="header-text">
                    <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Other Organizations Management</h4>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Manage all other organizations in the system</p>
                </div>
            </div>
            <div class="mt-4 md:mt-0 flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                <a href="{{ route('otherorganizations.create', $organization->slug) }}"
        ...
        <div class="premium-card-body">
            <div class="premium-table-container">
                <table class="premium-table" id="other-organizations-table">
                    <thead>
                        <tr>
                            <th width="10">SN</th>
                            ...
                            <th width="100">Established Date</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            var table_listing_table = $('#other-organizations-table').DataTable({
                ...
                "columns": [{
                    ...
                    },
                    {
                        "data": function(row) {
                            return row.short_name ?
                                `<span class="font-semibold">${row.short_name}</span>` :
                    ...
                    },
                    {
                        "data": function(row) {
                            return row.description ?
                                `<div class="max-w-7 whitespace-normal break-words flex items-center">
                    ...
                    },
                    {
                        "data": function(row) {
                            return row.image ?
                                `<img src="${row.image}" class="h-12 rounded" />` :
                                '<span class="text-gray-500">N/A</span>';
                        },
                        "name": "image",
                        "orderable": false
                    ...
                    {
                        "data": function(row) {
                            return row.established_date ?
                                `<span class="font-semibold">${row.established_date}</span>` :
                                '<span class="text-gray-500">N/A</span>';
                        },
                    ...
                    {
                        "data": function(row) {
                            const editUrl = "{{ route('otherorganizations.edit', [':slug', ':id']) }}"
                                .replace(':slug', "{{ $organization->slug }}")
                                .replace(':id', row.id);

                            var statusBtn =
                                `<button type="button" class="toggle-status-btn rounded-md border border-transparent p-2 text-center text-sm transition-all ${row.status === 'active' ? 'text-green-600 hover:bg-green-50' : 'text-red-600 hover:bg-red-50'}" data-id="${row.id}" data-name="${row.name}" data-status="${row.status}">
                                    <i class="fas ${row.status === 'active' ? 'fa-toggle-on' : 'fa-toggle-off'} w-5 h-5"></i>
                                </button>`;
            ...
            $(document).on('click', '.toggle-status-btn', function() {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ route('otherorganizations.index', $organization->slug) }}",
                    data: {
                        id: id,
                        status:1,
                    },
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                nepalConfirm.show({
                    ...
                    const url = "{{ route('otherorganizations.destroy', [':slug', ':id']) }}"
                        .replace(':slug', "{{ $organization->slug }}")
                        .replace(':id', id);
                    $.ajax({
                        url: url,
                    ...
                    nepalToast.info('Action Canceled', 'Organization deletion was canceled.');
                });
            });
        });
    </script>
