...
@section('content')
    @include('components.breadcrumb', [
        'breadcrumbs' => [['title' => 'blog', 'url' => null]],
    ])
    ...
    <div class="premium-card bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700">
        <div
            ...
            <div class="mt-4 md:mt-0 flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                <a href="{{ route('blog.create', $organization->slug) }}"
            ...
            <div class="flex items-center gap-4 flex-wrap">
                ...
                <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <div class="relative">
                        ...
                        <div class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-blog-none">
...
@push('scripts')
        ...
        $(document).ready(function() {
            ...
            <div class="flex items-center">
                <span class="font-semibold">${truncated}</span>
                    ...
                    {
                        "data": function(row) {
                            var plainText = $('<div>').html(row.name).text();
                            var truncated = plainText.length > 30 ? plainText.substring(0, 30) +
                                '...' : plainText;
                            return row.name ? `
            <div class="flex items-center">
                <span class="font-semibold">${truncated}</span>
                    ...
                    {
                        "data": function(row) {
                            var editUrl = "{{ route('blog.edit', ':id') }}".replace(':id', row.id);
                            var statusBtn =
                                `
   ...
   class="rounded-md border border-transparent p-2 text-center text-sm transition-all text-slate-600 hover:bg-slate-100 focus:bg-slate-100 active:bg-slate-100">
    <i class="fas fa-edit w-4 h-4"></i>
                                        ...
                                        <button type="button" class="rounded-md border border-transparent p-2 text-center text-sm transition-all text-slate-600 hover:bg-slate-100 focus:bg-slate-100 active:bg-slate-100 disabled:pointer-blog-none disabled:opacity-50 disabled:shadow-none delete-btn" data-id="${row.id}">
                ...
                "drawCallback": function(settings) {
                    ...
                    paginationHtml += `
                        <div class="page-item ${pageInfo.page === 0 ? 'opacity-50 cursor-not-allowed' : ''}">
                            <button class="page-link px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-l-md ${pageInfo.page === 0 ? 'pointer-blog-none' : ''}" data-page="${pageInfo.page - 1}">
                    ...
                    paginationHtml += `
                        <div class="page-item ${pageInfo.page === pageInfo.pages - 1 ? 'opacity-50 cursor-not-allowed' : ''}">
                            <button class="page-link px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-r-md ${pageInfo.page === pageInfo.pages - 1 ? 'pointer-blog-none' : ''}" data-page="${pageInfo.page + 1}">
            ...
            $('#custom-pagination').on('click', '.page-link:not(.pointer-blog-none)', function() {
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                nepalConfirm.show({
                    ...
                    nepalToast.info('Action Canceled', 'Blog deletion was canceled.');
                });
            });
        });
    </script>
