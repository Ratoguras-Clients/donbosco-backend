...
'breadcrumbs' => [['title' => 'Notice', 'url' => null]],
])
    ...
    <div
        ...
        <div class="premium-card-title flex items-center mb-4 md:mb-0">
            ...
            <div class="header-text">
                <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Notice Management</h4>
                <p class="text-sm text-gray-500 dark:text-gray-400">Manage all Notices in the system</p>
            </div>
        </div>
        <div class="mt-4 md:mt-0 flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
            <a href="{{ route('notices.create', $organization->slug) }}"
                    ...
                    "data": function(row) {
                        return `
                                <div class="flex items-center">
                                    ...
                                    ${row.is_published ? '<span class="inline-block w-3 h-3 rounded-full bg-green-500 ml-2"></span>' : '<span class="inline-block w-3 h-3 rounded-full bg-red-500 ml-2"></span>'}
        ...
        `;
                ...
                },
                {
                    "data": function(row) {
                        return `
                                <div class="flex items-center">
                                    <span class="font-semibold">${row.notice_date}</span>
                                </div>`;
                    },
                    "name": "notice_date",
                    "orderable": false
                },
                {
                    "data": function(row) {
                        return row.created_by ?
                            `<span class="role-badge px-2 py-1 text-xs rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300">${row.created_by}</span>` :
                ...
                {
                    "data": function(row) {
                        var editUrl = "{{ route('notices.edit', ['slug' => $organization->slug, 'id' => ':id']) }}".replace(':id', row.id);
                        var statusBtn =
                            `
    ...
    <i class="fas fa-edit w-4 h-4"></i>
        ...
        $(document).on('click', '.delete-btn', function() {
            ...
            nepalConfirm.show({
                ...
                const url = "{{ route('notices.destroy', ['slug' => $organization->slug, 'id' => ':id']) }}".replace(':id', id);
                $.ajax({
                    url: url,
                    type: "DELETE",
                    data: {
