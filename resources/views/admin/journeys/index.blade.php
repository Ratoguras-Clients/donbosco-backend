...
@section('content')
    @include('components.breadcrumb', [
        'breadcrumbs' => [['title' => 'Journeys', 'url' => null]],
    ])
        ...
        <div
            ...
            <div class="premium-card-title flex items-center mb-4 md:mb-0">
                ...
                <div class="header-text">
                    <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Journey Management</h4>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Manage all Journeys in the system</p>
                </div>
            </div>
            <div class="mt-4 md:mt-0 flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                <a href="{{ route('journeys.create', $organization->slug) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            <span class="px-2 py-1 text-xs rounded-full 
                ${row.is_active ? 'bg-green-600 text-white text-bold' : 'bg-red-600 text-white text-bold'}">
        ...
        `;
                    ...
                    {
                        "data": function(row) {
                            var editUrl = "{{ route('journeys.edit', ':id') }}".replace(':id', row
                                .id);
                            var statusBtn =
                                `
                                    <button type="button" class="toggle-status-btn rounded-md border border-transparent p-2 text-center text-sm transition-all ${row.is_active ? 'text-green-600 hover:bg-green-50' : 'text-red-600 hover:bg-red-50'}" data-id="${row.id}" data-name="${row.title}" data-status="${row.is_active}">
    ...
    <i class="fas fa-edit w-4 h-4"></i>
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                nepalConfirm.show({
                    ...
                    const url = "{{ route('journeys.destroy', ':id') }}".replace(':id', id);
                    $.ajax({
                        url: url,
                        type: "DELETE",
                        data: {
