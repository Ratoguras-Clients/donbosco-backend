...
@section('content')
    @include('components.breadcrumb', [
        'breadcrumbs' => [['title' => 'Missions', 'url' => null]],
    ])
        ...
        <div
            ...
            <div class="premium-card-title flex items-center mb-4 md:mb-0">
                ...
                <div class="header-text">
                    <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Mission Management</h4>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Manage all missions in the system</p>
                </div>
            </div>
    ...
    <div id="custom-modal" class="z-50 bg-black/50 fixed inset-0 flex items-center justify-center" style="display: none;">
        <div class="bg-white dark:bg-gray-700 rounded-lg shadow-lg w-full max-w-4xl p-4 md:p-5 relative">
            <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-600 pb-3">
                <h3 id="model-title" class="text-lg font-semibold text-gray-900 dark:text-white">Create Mission</h3>
                <span class="closeModal iconify cursor-pointer" data-icon="ic:baseline-close" data-width="24"></span>
            </div>

            <form action="{{ route('mission.store', $organization->slug) }}" class="mt-4 mb-0" id="staff-role-form">
                @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                    ...
                    <div class="col-span-2">
                        <label for="icon-search" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Search
                            Icon</label>
                        <div class="relative">
                            <div class="flex gap-2">
                                <div class="relative flex-1">
                                    <input type="text" id="icon-search"
                                        placeholder="Type to search (e.g. target, mountain, user)..."
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:text-white">
                                </div>
                                <input type="hidden" id="icon" name="icon" required>
                                <div
                                    class="w-11 h-11 flex-shrink-0 flex items-center justify-center border border-gray-300 dark:border-gray-500 rounded-lg bg-gray-50 dark:bg-gray-600">
                                    <span id="icon-preview" class="iconify text-2xl text-blue-600"
                                        data-icon="lucide:search"></span>
                                </div>
                            </div>
                            <div id="icon-suggestions"
                                class=" absolute z-50 w-full mt-1 max-h-48 overflow-y-auto bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl grid grid-cols-8 gap-1 p-2">
                            </div>
                        </div>
                    </div>
                    <div class="col-span-2">
                        ...
                        <textarea id="description" rows="4" placeholder="Write Mission description here" name="description"
                ...
                <div class="flex justify-between gap-3 py-3">
                    <button type="button"
                        class="closeModal text-white bg-red-700 hover:bg-red-800 rounded-lg text-sm px-5 py-2.5 inline-flex items-center">Close</button>
                    <button type="submit"
                        class="text-white bg-blue-700 hover:bg-blue-800 rounded-lg text-sm px-5 py-2.5 inline-flex items-center">Save
                        Mission</button>
                </div>
            </form>
        </div>
...
@push('scripts')
    <script>
        function modelOpen() {
            $('#custom-modal').show();
            $('#title').val('').focus();
            $('#model-title').text('Create Mission');
        ...
        function updateCreateButton(api) {
            var activeCount = 0;
            ...
            if (activeCount >= 3) {
                $('#openModal').prop('disabled', true)
                    ...
                    .attr('title', 'Maximum 3 active missions allowed. Deactivate one to add more.');
            } else {
        ...
        $(document).ready(function() {
                        ...
                        .then(data => {
                            ...
                            if (data.icons.length > 0) {
                                data.icons.forEach(icon => {
                                    html += `<div class="icon-option p-2 hover:bg-blue-50 dark:hover:bg-gray-600 rounded cursor-pointer flex justify-center" data-id="${icon}">
            ...
            var table_listing_table = $('#staff-role-table').DataTable({
                ...
                "columns": [{
                    ...
                    {
                        "data": function(row) {
                            let iconName = row.icon ? row.icon : 'lucide:target';
                            return `
                    ...
                    {
                        "data": function(row) {
                            var statusBtn = `<button type="button" class="toggle-status-btn rounded-md border border-transparent p-2 text-center text-sm transition-all ${row.is_active ? 'text-green-600 hover:bg-green-50' : 'text-red-600 hover:bg-red-50'}" data-id="${row.id}">
                                                <i class="fas ${row.is_active ? 'fa-toggle-on' : 'fa-toggle-off'} w-5 h-5"></i>
                                            </button>`;

                            return `<div class="flex space-x-2">
                                        <button type="button" class="rounded-md border border-transparent p-2 text-slate-600 hover:bg-slate-100 edit-btn"
                                            data-id="${row.id}" data-description="${row.description}"
                                            data-title="${row.title}" data-icon="${row.icon}">
                                            <i class="fas fa-edit w-4 h-4"></i>
                                        </button>
                                        <button type="button" class="rounded-md border border-transparent p-2 text-slate-600 hover:bg-slate-100 delete-btn" data-id="${row.id}">
                ...
                "drawCallback": function(settings) {
                    ...
                    // Update create button state based on active count
                    updateCreateButton(api);
                    ...
                    for (var i = Math.max(0, pageInfo.page - 2); i <= Math.min(pageInfo.pages - 1,
                            pageInfo.page + 2); i++) {
                        paginationHtml +=
            ...
            $('#staff-role-form').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('mission.store', $organization->slug) }}",
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                nepalConfirm.show({
                    ...
                    $.ajax({
                        url: "{{ route('mission.destroy', ['slug' => $organization->slug, 'id' => ':id']) }}".replace(':id', id),
                        type: "DELETE",
                        data: {
                            _token: "{{ csrf_token() }}"
                        },
