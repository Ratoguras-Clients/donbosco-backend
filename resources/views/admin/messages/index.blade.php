        ...
        <div
            ...
            <div class="premium-card-title flex items-center mb-4 md:mb-0">
                ...
                <div class="header-text">
                    <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Messages Management</h4>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Manage all Messages in the system</p>
                </div>
            </div>
            <div class="mt-4 md:mt-0 flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                <a href="{{ route('messages.create', $organization->slug) }}"
                ...
                <span class="font-semibold" title="${plainText}">${truncated}</span>
                    ...
                    },
                    {
                        "data": function(row) {
                            return `
                                <div class="flex items-center">
                                    <span class="font-semibold">${row.tenure}</span>
                    ...
                    {
                        "data": function(row) {
                            var editUrl = "{{ route('messages.edit', ':id') }}".replace(':id', row
                                .id);
                            var statusBtn =
                                `
    ...
    <i class="fas fa-edit w-4 h-4"></i>
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                nepalConfirm.show({
                    ...
                    const url = "{{ route('messages.destroy', ':id') }}".replace(':id', id);
                    $.ajax({
                        url: url,
                        type: "DELETE",
                        data: {
