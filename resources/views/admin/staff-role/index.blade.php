...
@section('content')
    @include('components.breadcrumb', [
        'breadcrumbs' => [['title' => 'Staff Role', 'url' => null]],
    ])
        ...
        <div
            ...
            <div class="premium-card-title flex items-center mb-4 md:mb-0">
                ...
                <div class="header-text">
                    <h4 class="text-lg font-semibold text-gray-800 dark:text-white">Staff Role Management</h4>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Manage all Staff Role in the system</p>
                </div>
            </div>
    ...
    <div id="custom-modal" class="z-50 bg-black/50 fixed inset-0 flex items-center justify-center" style="display: none;">
        <div class="bg-white dark:bg-gray-700 rounded-lg shadow-lg w-full max-w-4xl p-4 md:p-5 relative">
            ...
            <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-600 pb-3">
                <h3 id="model-title" class="text-lg font-semibold text-gray-900 dark:text-white">Create New Staff Role</h3>
            ...
            <form class="mt-4 mb-0" id="staff-role-form">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 pb-4 gap-6">
                    <div class="flex items-center">
                        <input type="checkbox" name="is_messageable" id="is_messageable" value="1"
                    ...
                    },
                    {
                        "data": function(row) {
                            ...
                            var statusBtn =
                                ...
                                `;
                            var messageableBtn = `
    <button type="button" class="toggle-messageable-btn rounded-md border border-transparent p-2 text-center text-sm transition-all ${row.is_messageable ? 'text-blue-600 hover:bg-blue-50' : 'text-gray-600 hover:bg-gray-50'}" data-id="${row.id}" data-name="${row.name}" data-messageable="${row.is_messageable}">
        <i class="fas ${row.is_messageable ? 'fa-toggle-on' : 'fa-toggle-off'} w-5 h-5"></i>
    </button>
`;
            ...
            // Form submission with loading state yes
            ...
            $(document).on('click', '.toggle-messageable-btn', function() {
                const id = $(this).data('id');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) {
                            nepalToast.success('Success', response.message ||
                                'Messageable status updated successfully.');
                            table_listing_table.ajax.reload();
                        } else {
                            nepalToast.error('Error', response.message ||
                                'An error occurred. Please try again.');
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                $('#model-title').text('Edit Staff Role');
                $('#name').val(name).focus();
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                nepalConfirm.show({
                    ...
                    const url = "{{ route('staff.destroy', ':id') }}".replace(':id', id);
                    $.ajax({
                        url: url,
                        type: "DELETE",
                        data: {
